...
import { regMap } from "./RegulationTable";
...
const RegulationModal = ({ isOpen, onClose, title, updateCourse, course, editReg }) => {
    ...
    const handleAddToRegArrayClick = (type) => {
        if (type === "POINTS") {
            ...
            setOutputRegulation([...outputRegulation, ...selectValue.map((s) => s.label)]);
            setSelectValue([]);
        }
    ...
    const isAddDisabled =
        title === "Prerequisites" || title === "Corequisites"
    ...
    return (
        <>
            <Modal id="regulation-modal" isOpen={isOpen} onClose={handleClose} size="full">
                <ModalOverlay />
                <ModalContent>
                    <ModalHeader>Add {regMap[title] && regMap[title].button}</ModalHeader>
                    <ModalCloseButton />

                    <Text p="20px">{regMap[title] && regMap[title].tooltip}</Text>
                            ...
                            <FormControl>
                                <Flex>
                                    ...
                                    <IconButton
                                        ...
                                        data-tip
                                        data-for="addTooltip"
                                        onClick={() => handleAddToRegArrayClick("COURSE")}
                                    />
                                </Flex>
                            </FormControl>
                        )}
