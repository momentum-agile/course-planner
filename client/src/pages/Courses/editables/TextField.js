...
const placeholders = {
    code: "e.g. SOFTENG 761",
    name: "e.g. Advanced Agile and Lean Software Development",
    description: "A longer description of the course",
}

const validCourseCodeRegex = /^([A-Za-z])*(\s?)([1-9][0-9][0-9])([A-Za-z]{0,3})$/;
const autoGenerateToolTip = "Enter a valid course code, e.g. SOFTENG 761, and click to get course data from the UoA API";
...
const TextField = ({ isRequired = false, name, title, value, isEditing, onChange, prefillCourse, setPrefill }) => {
    ...
    const handlePrefill = () => {
        ...
        prefillCourse(subject.trim(), courseCode.trim())
            ...
            .then(() => toast({ isClosable: true, title: "Course Auto-Filled", description: `Successfully auto-filled info for ${value}`, status: "success" }))
